...
void MessageDispatcher::dispatch(ClientConnection* c, net::Envelope& env)
{
    switch (env.msg_type()) {
        ...
        case net::MsgType::MSG_MOVE:
            m_roomManager.forwardMove(c, *env.mutable_move());
            break;
...
void MessageDispatcher::dispatchSetup(ClientConnection *c,const net::Setup &setup) {
    switch (setup.type()) {
        case net::Setup_MessageType_HOST_REQUEST:
            ...
            m_roomManager.joinRoom(c, setup.host_request().game_name(), setup.host_request().player_name());
        break;
        case net::Setup_MessageType_JOIN_REQUEST:
            m_roomManager.joinRoom(c, setup.join_request().game_name(), setup.join_request().player_name());
            c->setName(setup.host_request().player_name());
        break;
