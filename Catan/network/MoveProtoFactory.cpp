    ...
    switch (proto.payload_case()) {
      ...
      case catan::Move::kRollDice:
        ...
        case catan::Move::kMonopoly:
            return std::make_unique<MonopolyMove>(pId, static_cast<ResourceType>(proto.monopoly().resource()));
        ...
        case catan::Move::kYearOfPlenty:
            return std::make_unique<YearOfPlentyMove>(pId,
                static_cast<ResourceType>(proto.year_of_plenty().first_resource()),
                static_cast<ResourceType>(proto.year_of_plenty().second_resource()));

        case catan::Move::kBankTrade:
            return std::make_unique<BankTradeMove>(pId,
                static_cast<ResourceType>(proto.bank_trade().give()),
                static_cast<ResourceType>(proto.bank_trade().receive()));

        case catan::Move::kPlayerTradeRequest: {
            ResourcePack give, receive;
            for (auto const& [res, count] : proto.player_trade_request().give())
                give[static_cast<ResourceType>(res)] = count;
            for (auto const& [res, count] : proto.player_trade_request().receive())
                receive[static_cast<ResourceType>(res)] = count;
            return std::make_unique<PlayerTradeRequestMove>(pId, give, receive);
        }

        case catan::Move::kPlayerTradeResponse:
            return std::make_unique<PlayerTradeResponseMove>(pId, proto.player_trade_response().trade_id());

        case catan::Move::kPlayerTradeAccept:
            return std::make_unique<PlayerTradeAcceptMove>(pId,
                proto.player_trade_accept().accepted_player_id(),
                proto.player_trade_accept().trade_id());
        default:
            return nullptr;
    }
}