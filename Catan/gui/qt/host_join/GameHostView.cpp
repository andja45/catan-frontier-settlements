#include "GameHostView.h"

GameHostView::GameHostView(const std::string &gameName, QWidget *parent)
    : QWidget(parent), m_gameName(QString::fromStdString(gameName))
{
    setWindowTitle(QString("Hosting Game: %1").arg(m_gameName));
    ...
    connect(m_startButton, &QPushButton::clicked, this, [this]() {
        ...
        emit startGameRequested(
            m_gameName,
            m_maxPlayersSpin->value(),
            m_pointsToWinSpin->value(),
            mapType
            );
    });

}
QGroupBox* GameHostView::setMapSelect()
{
...
void GameHostView::addPlayer(const QString &playerName)
{
    ...
    connect(widget, &PlayerListItem::kickRequested,
            this, &GameHostView::kickPlayerRequested);
}
void GameHostView::removePlayer(const QString &playerName)
{
...
void GameHostView::clearPlayers()
{
    m_playerList->clear();
    addAddBotEntry();
}
void GameHostView::setPlayerCount(int count)
{
    m_playerCountLabel->setText(
        QString("Players joined: %1").arg(count)
        );
}
void GameHostView::addAddBotEntry()
{
    auto *item = new QListWidgetItem(m_playerList);
    auto *widget = new AddBotListItem;
    ...
    connect(widget, &AddBotListItem::addBotRequested,
            this, &GameHostView::addBotRequested);
}
/*
#TODO: za kontroler posle
connect(controller, &GameController::playerJoined,
        hostView, &GameHostView::addPlayer);

connect(controller, &GameController::playerLeft,
        hostView, &GameHostView::removePlayer);

connect(hostView, &GameHostView::kickPlayerRequested,
        controller, &GameController::kickPlayer);

connect(hostView, &GameHostView::addBotRequested,
        controller, &GameController::addBot);

connect(hostView, &GameHostView::customMapSelected,
        controller, &GameController::loadCustomMap);
*/
