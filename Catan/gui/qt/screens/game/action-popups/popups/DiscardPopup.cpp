...
DiscardPopup::DiscardPopup(Player* player, QWidget* parent)
    : FloatingPanel( parent,Qt::Dialog), m_player(player)
{
...
void DiscardPopup::openAtGlobal(Player* player,Bank* bank, const QPoint& globalPos) {
    restart();
...
void DiscardPopup::closeEvent(QCloseEvent* event)
{
    event->ignore();
}

void DiscardPopup::showEvent(QShowEvent* event)
{
    m_holdings = m_player->getResources();
    updateUiState();
}