...
// If you have a shared list already, use it instead:
    ...
    : FloatingPanel(Qt::Popup, parent), m_bank(bank)
{
    ...
    // only style controls, not QFrame
    setStyleSheet(R"(
        QPushButton {
            padding: 6px 10px;
            border-radius: 8px;
    ...
    for (auto rt : m_types) {
        m_bankResources[rt] = 19;     // default; replace via setBankResources()
...
void YearOfPlentyPopup::setBankResources(const std::map<ResourceType,int>& bankResources) {
    m_bankResources = bankResources;
    updateUiState();
}
...
void YearOfPlentyPopup::updateUiState() {
    ...
    m_playBtn->setEnabled(sel == 2);

    if (sel < 2) m_hint->setText(QString("Pick %1 more card(s). (Left/Right click)").arg(2 - sel));
    else if (sel == 2) m_hint->setText("Ready. Click Play.");
    ...
    for (int i = 0; i < (int)m_cards.size(); ++i) {
        ...
        // If you want: dim card when bank has 0. For now ignore as requested.
        (void)bankHasAny;
    }
}
