...
static constexpr int STACK_TOP_MARGIN  = 20;
static constexpr int STACK_LEFT_MARGIN = 20;
static constexpr int STACK_SPACING     = 10;

static QVector<TradeOfferedPopup*> s_openPopups;
...
bool TradeOfferedPopup::isOfferAcceptable() const{
    ...
    /*for (QCard* card : m_giveCards) {
        const auto& spec = card->spec();
        auto it = m_playerResources.find(spec.resource);
        if (it == m_playerResources.end())
            return false;

        if (spec.countBadge > it->second)
            return false;
    }
    return true;*/
}
void TradeOfferedPopup::relayoutAll()
{
    int y = STACK_TOP_MARGIN;

    for (TradeOfferedPopup* p : s_openPopups) {
        if (!p) continue;
        p->move(STACK_LEFT_MARGIN, y);
        if (y+p->height() + STACK_SPACING<=parentWidget()->height() - p->height() - STACK_TOP_MARGIN-100)
            y += p->height() + STACK_SPACING;
    }
}
void TradeOfferedPopup::stackAndShow()
{
    show();
    raise();
    relayoutAll();
...
void TradeOfferedPopup::closeEvent(QCloseEvent* e)
{
    s_openPopups.removeOne(this);
    relayoutAll();
    QWidget::closeEvent(e);
}
bool TradeOfferedPopup::eventFilter(QObject *obj, QEvent *event)
{
...
void TradeOfferedPopup::paintEvent(QPaintEvent*) {
    QPainter p(this);
