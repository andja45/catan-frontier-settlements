...
std::unordered_map<int, std::vector<int> > LongestRoadRule::createRoadGraph(GameSession &session, Player &p) {
    ...
    for (auto *road: roads) {
        ...
        const EdgeId id = road->getEdgeId();
        roadGraph[id];
        for (Edge *adj: board.getIncidentContinuousEdges(id)) {
            if (!adj) {
                continue;
            }
            if (adj->getOwner() != p.getPlayerId()) {
                continue;
            }
...
int LongestRoadRule::dfsLongest(EdgeId current, const std::unordered_map<EdgeId, std::vector<EdgeId> > &graph,
...
std::pair<int, int> LongestRoadRule::bfsFarthest(int start, const std::unordered_map<int, std::vector<int> > &graph,
    ...
    while (!q.empty()) {
        ...
        if (localVisited.find(node) != localVisited.end())
            continue;
